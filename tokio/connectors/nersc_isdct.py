...
Process and aggregate the output of Intel Data Center Tool for SSDs outputs in
the format generated by NERSC's daily script.  The NERSC infrastructure runs
...
(e.g., nid01234/*.txt).  There is also an optional timestamp file contained in
the toplevel directory.  Also processes .tar.gz versions of these collected
metrics.
"""
...
class NerscIsdct(dict):
    ...
    def save_cache(self, output_file=None):
        ...
        Args:
            output_file (str or None): Path to file to which json should be
                written.  If None, write to stdout.
        """
    ...
    def diff(self, old_isdct, report_zeros=True):
        ...
        Args:
            ...
            report_zeros (bool): Report all counters even if they showed no
                change if True
    ...
    def _synthesize_metrics(self):
        ...
        presented by ISDCT and populate their keys in self
        """
...
def _merge_parsed_counters(parsed_counters_list):
    ...
    Aggregate counters from each record based on the NVMe device serial number,
...
def _rekey_smart_buffer(smart_buffer):
    """Convert SMART vlaues associated with one register into unique counters
...
def walk_file_collection(input_source):
    ...
    Yields:
        tuple: yields the following attributes for a member of `input_source`:
