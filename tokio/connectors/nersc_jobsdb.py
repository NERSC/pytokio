...
def set_concurrent_jobs_cached(darshan_log, value):
    ...
    """
    subprocess.call(['setfattr', '-n', _CONCURRENT_JOBS_XATTR, '-v', '"%d"' % value, darshan_log])
    return

def get_concurrent_jobs(darshan_log):
    value = get_concurrent_jobs_cached(darshan_log)
    if value is None:
        darshan_perf = extract_darshan_perf.extract_darshan_perf(darshan_log)
...
if __name__ == "__main__":
    for i in sys.argv[1:]:
        print "%6d %s" % (get_concurrent_jobs(i), i)
